...
class Network:
    ...
    def backpropagation(self):
        for i in range(len(self.layers[-1].nodes)):
            ...
            print('single predictions', prediction * self.label_list)
            print('dot', prediction @ self.label_list)
            loss = -(np.log(prediction @ self.label_list))
            print('loss', loss)
            # loss -= 1
            ...
            self.layers[-1].nodes[i].delta = loss
        ...
        for i in range(len(self.layers) - 2, 0, -1):
            for j in range(len(self.layers[i].nodes)):
                ...
                for k in range(len(self.layers[i + 1].nodes)):
                    sum += self.layers[i + 1].nodes[k].weights[j] * self.layers[i + 1].nodes[k].delta
                # self.layers[i].nodes[j].delta = activation_derivative * sum
                self.layers[i].nodes[j].delta = sum
